...
'写入总所日志'
function writerzzs(NFDT_SuoZhang,NFDT_ChuShi,NFDT_HuWeiDui,NFDT_YiYuan,NFDT_ShuiDianBan,NFDT_SiJi)
	sqlcmdd = ("select * from rizhi where datediff('d',rzdate,'"&date()&"') = 0")			'查询有没当天记录
	set rss=server.createobject("adodb.recordset")
''	response.write(text)
	rss.Open sqlcmdd,Louconn,1,1
		if rss.recordcount<>0 then
					text = ("update rizhi set szzb='"&NFDT_SuoZhang&"',cszb='"&NFDT_ChuShi&"',hwdzb='"&NFDT_HuWeiDui&"',yyzb='"&NFDT_YiYuan&"',sdbzb='"&NFDT_ShuiDianBan&"',sjzb='"&NFDT_SiJi&"'")			'更新记录
					set rs=server.createobject("adodb.recordset")
					rs.Open text,Louconn,1,3
					set rs=nothing
					rs.close
					...
					tt=date()
					text = ("insert into rizhi(szzb,cszb,hwdzb,yyzb,sdbzb,sjzb,rzdate)values('"&NFDT_SuoZhang&"','"&NFDT_ChuShi&"','"&NFDT_HuWeiDui&"','"&NFDT_YiYuan&"','"&NFDT_ShuiDianBan&"','"&NFDT_SiJi&"','"&tt&"')")		'插入新记录
''					response.write(date())
''					response.end
					Louconn.execute text
					rs.close
